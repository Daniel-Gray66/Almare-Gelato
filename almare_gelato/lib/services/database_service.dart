...
class DatabaseHelper {
  ...
  /// Gets the current stamp data from the database
  /// Returns a map with 'stamps' (int) and 'colors' (String) keys
  ...
  Future<void> addStamp() async {
    ...
    if (newStamps > maxStamps) {
      // Reset stamps completely with empty colors array
      await resetStamps();
      print('Resetting stamp!');
    } else {
      // Make sure we're passing the correct colors data
      final String colors = stampData['colors'] as String? ?? '[]';
      await updateStampData(newStamps, colors);
    }
  }
} 